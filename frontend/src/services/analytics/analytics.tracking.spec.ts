...
describe('analytics.tracking', () => {
  ...
  it('trackConversionCheckoutStart', async () => { const m = await import('./analytics.tracking'); m.trackConversionCheckoutStart('plan', 'id', 10); expect(mockTrack).toHaveBeenCalled() })
  it('trackConversionCheckoutSuccess', async () => { const m = await import('./analytics.tracking'); m.trackConversionCheckoutSuccess('plan', 'id', 10); expect(mockTrack).toHaveBeenCalled() })
  ...
  it('trackConversionPaymentConfirmed', async () => { const m = await import('./analytics.tracking'); m.trackConversionPaymentConfirmed(10); expect(mockTrack).toHaveBeenCalled() })
  it('trackConversionPaymentSuccess', async () => { const m = await import('./analytics.tracking'); m.trackConversionPaymentSuccess(10); expect(mockTrack).toHaveBeenCalled() })
